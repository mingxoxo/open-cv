...
//열거형 - 순서대로 CV_OBD_TRAIN = 0, CV_OBD_TEST = 1
...
//getectorGroundTruth에서 2차원 float 목록을 내림차순으로 정렬하는 데 사용
...
public:
   ...
   ~VocData() {}
   /* 객체 클래스가 지정된 여러 이미지에 대한 분류/객체 데이터를 반환하는 기능 */
   ...
   void getClassObjects(const string& obj_class, const ObdDatasetType dataset, vector<ObdImage>& images, vector<vector<ObdObject> >& objects, vector<vector<VocObjectData> >& object_data, vector<VocGT>& ground_truth);
  	/* 이미지 ID가 지정된 단일 이미지에 대해 개체 데이터를 반환하는 기능 */
...
void VocData::getClassImages_impl(const string& obj_class, const string& dataset_str, vector<ObdImage>& images, vector<char>& object_present)
{
   ...
   cout <<"filename"<< gtFilename.find("%s") << endl;
   ...
   const string& resPath)
{
   ...
   for (size_t i = 0; i < images.size()-3200; i++)
   {
...
int main(int argc, char** argv)
{
   ...
   const string vocPath = "C:/Users/gkseo/source/repos/Project15/Project15/training/VOCdevkit/VOC2010", resPath = "C:/Users/gkseo/source/repos/Project15/Project15/res";
